...
fishData <- data.frame(read.csv("data/iDigBioProcessedFishOccurrences_SingerAndOwens.csv"))
fishData <- fishData |> 
  select(iDigBio_dwc_decimalLongitude, 
         iDigBio_dwc_decimalLatitude, 
         SOcurated_depth, SOcurated_depthUncertainty,
         SOcurated_minimumDepthInMeters,
         SOcurated_maximumDepthInMeters,
         WoRMS_scientificName, 
         WoRMS_family,
         SOcurated_bibliographicCitation) |> 
  drop_na(iDigBio_dwc_decimalLongitude, 
         iDigBio_dwc_decimalLatitude, 
         SOcurated_depth, SOcurated_depthUncertainty,
         SOcurated_minimumDepthInMeters,
         SOcurated_maximumDepthInMeters,
...
fishData <- vect(fishData, 
                            geom = c("iDigBio_dwc_decimalLongitude", 
                                     "iDigBio_dwc_decimalLatitude"))
crs(fishData) <- crs("EPSG:4326")
...
fishData <- crop(fishData, gomShapefile)

land <- vect(rnaturalearth::ne_countries(scale = "small", 
                                         returnclass = "sf")[1])
land <- aggregate(land)
land <- project(land, crs("EPSG: 4326"))
landFish <- is.related(fishData, land, "within")
fishData <- fishData[!landFish,]
...
GOM <- vect(HexPols2)
crs(GOM) <- crs(land)
GOMland <- is.related(GOM, gomShapefile, "intersects")
GOM <- GOM[GOMland,]
```
...
```{r sampling density}
# Gridding it
point_density <- terra::extract(x = GOM, y = fishData)
fishData$hexID <- point_density[,2]
...
GOM$bathymetry <- terra::extract(x = bathymetry, y = GOM, fun = min)[,2]
...
for (i in 1:length(GOM)){
  ...
  if(any(refs)){
    maxDepth <- max(fishData[refs,"SOcurated_maximumDepthInMeters"], 
                    na.rm = TRUE)
  } else {
    maxDepth <- NA
  }
...
fishData <- fishData[fishData$SOcurated_depthUncertainty < 100,]
...
fishData <- data.frame(fishData) |> 
  filter(grepl('\\s', WoRMS_scientificName)) |> # Remove entries not ID'ed to species
  drop_na(WoRMS_scientificName)
...
while (test){
    ...
    tmp <- hist(fishData$SOcurated_depth,  
                breaks = breakNumber, 
                plot = F)
...
diversityStats <- function(x, binNumber){
  ...
  breaks <- seq(min(x$SOcurated_depth), 
              max(x$SOcurated_depth), 
              (max(x$SOcurated_depth) - min(x$SOcurated_depth))/(binNumber))
  
  # Species richness per bin
  depthDiversityCount <- NULL
  effort <- NULL
  for(i in 2:length(breaks)){
    temp <- x %>% filter(SOcurated_depth >= breaks[i-1], 
                         breaks[i] >= SOcurated_depth)
    depthDiversityCount <- c(depthDiversityCount,
                             length(unique(temp$WoRMS_scientificName)))
    effort <- c(effort, nrow(temp))
  }
  x$SOcurated_depthBin <- cut(x$SOcurated_depth, breaks=breaks, 
  ...
  depthComm <- data.frame(table(data.frame(x[c("SOcurated_depth", 
                                               "WoRMS_scientificName")])))
  depthComm <- as.data.frame.matrix(xtabs(Freq ~ SOcurated_depth + WoRMS_scientificName, 
                                          depthComm))
...
calcEffortAndAlpha <-function(rawData){
  effortAndAlpha <- data.frame(table(rawData$data$SOcurated_depthBin))
  for(i in 1:nrow(effortAndAlpha)){
    binCommunity <- rawData$data[rawData$data$SOcurated_depthBin == effortAndAlpha$Var1[i],]
    binCommunity <- binCommunity[complete.cases(binCommunity$SOcurated_depthBin),]
    effortAndAlpha$alpha[i] <- length(unique(binCommunity$WoRMS_scientificName))
  }
...
fishDatSim$data <- fishDatSim$data[!is.na(fishDatSim$data$SOcurated_depthBin),]
fishDatSim <- lapply(1:100, FUN = function(x) {
    fishDatSim$data$SOcurated_depthBin <- sample(fishDatSim$data$SOcurated_depthBin)
    return(fishDatSim)
  })
...
First, we extract the species richness and record counts for each depth bin in each hexagon. 

```{r Sampling effort versus species richness}
breaks <- tenPerBin$breaks
effortAndAlpha3D <- NULL
for(i in 2:length(breaks)){
  temp <- fishData %>% 
    filter(SOcurated_depth >= breaks[i-1], breaks[i] >= SOcurated_depth) %>% 
    group_by(hexID) %>% 
    summarise(alpha = n_distinct(WoRMS_scientificName), count = n())
  
  effortAndAlpha3D <- rbind(effortAndAlpha3D, 
                      data.frame(temp, depthBin = paste(round(breaks[i-1]), "to",
                                               round(breaks[i]))))
}
```

### Extracting environmental correlates

It was suggested by Katherine Richardson, Owens's colleague at the University of Copenhagen, that the bump in the diversity decay curve could be the result of thermoclines and/or pycnoclines. We used temperature and salinity data from the World Ocean Atlas 2018 dataset to investigate this idea, extracting mean temperature and salinity for each depth bin within each hexagon. We also examined apparent oxygen utilization and nitrogen concentration as potential correlates.
...
envDatExtract <- function(envRaster, mShp, breaks){
  ...
  for(i in 1:length(mShp)){
    grid <- mSampling3D(occs = dummyOccs, 
                        envBrick = envRaster, 
                        mShp = mShp[i], verbose = F)
    allData <- cbind(grid, 
                     xyzSample(grid, envBrick = envRaster, verbose = F))
    colnames(allData) <- c("longitude", "latitude", "depth", "Variable")
    
    # Assign points to bins and calculate summary stats
    sumData <- NULL
    for(j in 2:length(breaks)){
      temp <- allData %>% filter(depth >= breaks[j-1], breaks[j] >= depth)
      sumData <- rbind(sumData,
                       data.frame(Variable = mean(temp$Variable),
                                  depthBin = paste(round(breaks[j-1]), "to",
                                                round(breaks[j]))))
    }
    catData <- rbind(catData, data.frame(hexID = i, sumData))
  }
  return(catData)
}

envtData <- rast("~/Dropbox/DeepFishOccurrences/data/EnvironmentalData/ProcessedEnvtData/temperature.tif") %>%
  project(crs(gomShapefile)) %>% 
  crop(gomShapefile) %>% 
  mask(mask = gomShapefile) %>% 
  envDatExtract(mShp = GOM, breaks = tenPerBin$breaks)
colnames(envtData) <- c("hexID", "Temperature", "depthBin")

envtData <- merge(envtData, rast("~/Dropbox/DeepFishOccurrences/data/EnvironmentalData/ProcessedEnvtData/salinity.tif")  %>% 
  project(crs(gomShapefile)) %>% 
  crop(gomShapefile) %>% 
  mask(mask = gomShapefile) %>% 
  envDatExtract(mShp = GOM, breaks = tenPerBin$breaks))
colnames(envtData)[ncol(envtData)] <- "Salinity"

envtData <- merge(envtData,
                  rast("~/Dropbox/DeepFishOccurrences/data/EnvironmentalData/ProcessedEnvtData/AOU.tif") %>%
  project(crs(gomShapefile)) %>% 
  crop(gomShapefile) %>% 
  mask(mask = gomShapefile) %>% 
  envDatExtract(mShp = GOM, breaks = tenPerBin$breaks))
colnames(envtData)[ncol(envtData)] <- "AOU"

envtData <- merge(envtData,
                  rast("~/Dropbox/DeepFishOccurrences/data/EnvironmentalData/ProcessedEnvtData/nitrate.tif") %>%
  project(crs(gomShapefile)) %>% 
  crop(gomShapefile) %>% 
  mask(mask = gomShapefile) %>% 
  envDatExtract(mShp = GOM, breaks = tenPerBin$breaks))
colnames(envtData)[ncol(envtData)] <- "Nitrate"

envtData$maxBinDepth <- as.numeric(unlist(strsplit(as.character(envtData$depthBin),
                                                         " "))[seq(from = 3, 
                                                                   to = nrow(envtData) * 3, 
                                                                   by = 3)])
mergedData <- merge(envtData, effortAndAlpha3D[,c(1,4,2,3)])
```

Once the data were extracted, we modeled species richness within each depth bin within each hexagon as a function of the number of records, depth, temperature, salinty, AOU, and nitrate. Given the extreme right skew of the data, we used a Poisson distribution. Temperature, salinity, and nitrate had strong variance inflation factors. Once these variables were removed, there was a strong positive correlation between species richness and counts, and a weak negative correlation between species richness and the maximum depth of the bin (both statistically significant). 

```{r species richness model}
richnessModel <- glm(data = mergedData, alpha ~ maxBinDepth + count + Temperature + Salinity + AOU + Nitrate, poisson(link="identity"))
summary(richnessModel)
vif(richnessModel)

richnessModel <- glm(data = mergedData, alpha ~ maxBinDepth + count + AOU,
                     poisson(link="identity"))
summary(richnessModel)
vif(richnessModel)

convenienceEffect <- lm(data = mergedData, count ~ maxBinDepth)
summary(convenienceEffect)
```
...
```{r, fig.height = 10}
effortAndAlphaLong <- reshape2::melt(mergedData[,c("maxBinDepth", "alpha",
                                                   "count","Temperature", 
                                                   "Salinity","AOU", "Nitrate")], 
                                      id.vars = "maxBinDepth")
effortAndAlphaLong <- aggregate(effortAndAlphaLong, value ~ maxBinDepth + variable, mean)

P1 <- ggplot(effortAndAlphaLong, aes(maxBinDepth, value,
...
print(paste0("There are ", length(unique(fishData$WoRMS_scientificName)), 
...
ggplot()+
  ...
  #geom_hline(aes(yintercept = c(200,1000), col = "darkred"), lty = "dashed", show.legend = F) +
  #geom_vline(aes(xintercept = c(200,1000), col = "darkred"), lty = "dashed", show.legend = F) +
  geom_linerange(data=depthRanges,
                  aes(x=depthMin, 
                      ymin=depthMin, 
                      ymax=depthMax), 
                  linewidth=.5, alpha=0.5) +
  scale_x_continuous(limits = c(0, 4350)) +
  scale_y_continuous(limits = c(0, 7150)) +
  theme_bw()
```
