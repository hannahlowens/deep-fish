...
# Summary statistics for Gulf of Mexico analysis
...
for(dat in allDat){
  P1 <- P1 +
    geom_line(data = dat, aes(x = Depth, y = Alpha, group = 1), alpha=0.01)
  medProp <- c(medProp, median(dat$prop))
}

finalPlot <- P1 +
  geom_line(data = effortAndAlpha, aes(x = Depth, y = Alpha, group = 1), col = "red") +
  theme_Publication() + 
...
## Possible correlates with alpha diversity
...
effortAndAlpha$Temperature <- rast("~/Dropbox/MARDIGRA/data/EnvironmentalData/ProcessedEnvtData/temperature.tif") %>%
  ...
  envDatExtract(mShp = world, breaks = tenPerBin$breaks)
