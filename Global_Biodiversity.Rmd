...
for (i in 1:length(oceans)){
  ...
  if(any(refs)){
    maxDepth <- max(fishHorizonComplete[refs,"SOcurated_maximumDepthInMeters"], na.rm = TRUE)
  } else {
    maxDepth <- NA
  }
...
oceansFiltered <- oceans %>% 
  filter(!is.na(samplingRatio), samplingRatio >= .95)
print(paste0("There are ", nrow(oceansFiltered), " cells in the dataset that are sampled to at least 95% depth."))

oceansFiltered <- oceans %>% 
  filter(!is.na(samplingRatio), samplingRatio >= 1.0)
print(paste0("There are ", nrow(oceansFiltered), " cells in the dataset that are sampled to full depth."))
```
...
fishData <- fishData[str_detect(fishData$WoRMS_scientificName, " sp\\.", negate = TRUE),]
fishData <- fishData[str_detect(fishData$WoRMS_scientificName, "species", negate = TRUE),]
...
while (test){
    ...
    tmp <- hist(fishData$SOcurated_depth,  breaks = breakNumber, plot = F)
...
diversityStats <- function(x, binNumber){
  ...
  for(i in 2:length(breaks)){
    temp <- x %>% filter(SOcurated_depth >= breaks[i-1], breaks[i] >= SOcurated_depth)
...
effortAndAlpha <- calcEffortAndAlpha(rawData = zeroEmpty)
...
effortAndAlpha$Temperature <-
  rast("~/Dropbox/DeepFishOccurrences/data/EnvironmentalData/ProcessedEnvtData/temperature.tif") %>%
  project(crs(world)) %>% 
  crop(world, mask = TRUE) %>% 
  envDatExtract(mShp = world, breaks = zeroEmpty$breaks)
effortAndAlpha$Salinity <- rast("~/Dropbox/DeepFishOccurrences/data/EnvironmentalData/ProcessedEnvtData/salinity.tif")  %>% 
  project(crs(world)) %>% 
  crop(world, mask = TRUE) %>% 
  envDatExtract(mShp = world, breaks = zeroEmpty$breaks)
effortAndAlpha$AOU <- rast("~/Dropbox/DeepFishOccurrences/data/EnvironmentalData/ProcessedEnvtData/AOU.tif") %>%
  project(crs(world)) %>% 
  crop(world, mask = TRUE) %>% 
  envDatExtract(mShp = world, breaks = zeroEmpty$breaks)
effortAndAlpha$Nitrate <- rast("~/Dropbox/DeepFishOccurrences/data/EnvironmentalData/ProcessedEnvtData/nitrate.tif") %>%
  project(crs(world)) %>% 
  crop(world, mask = TRUE) %>% 
  envDatExtract(mShp = world, breaks = zeroEmpty$breaks)
```
